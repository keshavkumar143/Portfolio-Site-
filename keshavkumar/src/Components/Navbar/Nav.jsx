...
function Nav() {
  ...
  return (
    <motion.nav 
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-black/80 backdrop-blur-md py-2 shadow-lg' : 'bg-transparent py-4'}`}
      initial={{ y: -100 }}
      animate={{ y: 0 }}
      transition={{ type: 'spring', stiffness: 120, damping: 20, delay: 0.2 }}
    >
      <div className="NavContainer flex justify-between items-center container mx-auto px-6">
        <motion.div className="logo cursor-pointer" whileHover={{ scale: 1.05 }}>
          <Link to="/">
